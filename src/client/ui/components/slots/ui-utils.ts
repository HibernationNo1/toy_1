import type { SlotLayer } from "./types";
import { SLOT_Z_INDEX_RANGES } from "./constants";

export const clamp = (value: number, min: number, max: number) => math.max(min, math.min(max, value));

export const resolveZIndex = (layer: SlotLayer | undefined, value?: number) => {
  const range = SLOT_Z_INDEX_RANGES[layer ?? "base"];
  if (value !== undefined) {
